...
import 'package:flutter_comic/view/pages/comic_reader_view.dart';
...
class _ComicReaderPageState extends State<ComicReaderPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        children: <Widget>[
          !_loading
              ? false // Provider.of<AppSetting>(context).comicVerticalMode
          ...
          Positioned(
            child: true // Provider.of<AppSetting>(context).comicReadShowstate
                ? Container(
                    ...
                    child: Text(
                      _loading
                          ...
                          : false // Provider.of<AppSetting>(context).comicVerticalMode
                              ...
                              : "${_currentItem.chapterTitle}  $_selectIndex/${_detail.pageUrl.length}  $_networkState  $_batteryStr 电量",
          ...
          AnimatedPositioned(
            ...
            child: Container(
              padding: EdgeInsets.only(
                  top:
                      true // Provider.of<AppSetting>(context).comicReadShowStatusBar
                          ? 0
  ...
  void nextPage() async {
    if (_pageController.page == 1) {
      ...
      setState(() {
        _selectIndex = _detail.pageUrl.length;
  ...
  void previousPage() {
    if (_pageController.page > _detail.pageUrl.length) {
      nextChapter();
    } else {
      setState(() {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: ComicView.builder(
          reverse: false, // Provider.of<AppSetting>(context).comicReadReverse,
          builder: (BuildContext context, int index) {
            if (index > 0 && index <= _detail.pageUrl.length) {
              return PhotoViewGalleryPageOptions(
                filterQuality: FilterQuality.high,
                imageProvider: CachedNetworkImageProvider(
                  _detail.pageUrl[index - 1],
          ...
          itemCount: _detail.pageUrl.length + 3,
  ...
  Widget getExtraPage(int index) {
    ...
    if (index == _detail.pageUrl.length + 1) {
      return Container();
      // return createTucao(24);
    }
    if (index == _detail.pageUrl.length + 2) {
      return Center(
        child: Text(
    ...
    return InkWell(
      ...
      child: EasyRefresh(
        ...
        child: ListView.builder(
            itemCount: _detail.pageUrl.length + 1,
            controller: _scrollController,
            itemBuilder: (ctx, i) {
              if (i == _detail.pageUrl.length) {
                return Container();
                // return createTucao(24);
              } else {
                var f = _detail.pageUrl[i];
                return Container(
                  color: Colors.black,
  ...
  //                   }),
  //               !true //Provider.of<AppSetting>(context).comicSystemBrightness
  //                   ? Row(
  //                       children: <Widget>[
  //                         SizedBox(width: 12),
  //                         Icon(
  ...
  bool _loading = false;
  ListComicChapterDetailResponse _detail;
  ...
  Future loadData() async {
    try {
      ...
      ListComicChapterDetailResponse detail =
          await AppviewServiceClientProxy.getInstance().listComicChapterDetail(
              ListComicChapterDetailRequest(
                chapterId: _currentItem.chapterId,
